import gymnasium as gym
import ale_py
import torch

from agent import RainbowAgent


gym.register_envs(ale_py)

env = gym.make(
    "ALE/Breakout-v5",
    obs_type="grayscale",
    render_mode="human",
    repeat_action_probability=0,
)
agent = RainbowAgent(1e-4, env.action_space.n)  # pyright: ignore
agent.model.load_state_dict(
    torch.load(
        "weights/ep_16000.pt", weights_only=False, map_location=torch.device("cpu")
    )
)
agent.model.eval()

s, _ = env.reset()
last_frames = [torch.tensor(s) for _ in range(3)]
s, _, _, _, _ = env.step(
    1
)  # to start the game (rainbow agent has been trained like that to speed up training)
last_frames.append(torch.tensor(s))
for i in range(10000):
    phi_t = agent.preprocess_frames(torch.stack(last_frames))
    action = agent.get_action(phi_t.unsqueeze(0))

    new_s, _, done, _, info = env.step(action)

    last_frames.pop(0)
    last_frames.append(torch.tensor(new_s))

    if info["lives"] == 4 or done:
        break
